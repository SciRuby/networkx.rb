...
require 'pry'
module NetworkX
  def self.astar_path(g, source, target, heuristic=nil)
    warn 'A* is not implemented for MultiGraph and MultiDiGraph'
    raise ArgumentError, 'Either source or target is not in graph' unless g.node?(source) && g.node?(target)
    count = ->(i) { i + 1 }
    i = -1
    heuristic ||= (->(_u, _v) { 0 })
  ...
  def self.astar_path_length(g, source, target, heuristic=nil)
    raise ArgumentError, 'Either source or target is not in graph' unless g.node?(source) && g.node?(target)
    path = astar_path(g, source, target, heuristic)
    path_length = 0
    (1..(path.length - 1)).each { |i| path_length += (g.adj[path[i - 1]][path[i]][:weight] || 1) }
    path_length
  end
end
